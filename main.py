...
while True:
    ...
    success, frame = cap.read()
    frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)
    # box = cv2.selectROI("selected image", frame)
    # tracker.init(frame, box)
    # backup.init(frame, box)
    if not success:
        break
    ...
    if ret:
        ...
        print("thresh is: ", blink_thresh)
        average.append(ave)
        if len(average) == 11:
            del(average[0])
        #blink_thresh = 0.95*(sum(average)/len(average))
        blink_thresh = 1.15 * min(average)
